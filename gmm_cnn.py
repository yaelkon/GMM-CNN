...
class GMM_CNN(Encoder):
    ...
    def compile_model(self, options=None):
        ...
        self.keras_model.compile(optimizer=self.optimizer, loss=self.losses, metrics=self.metrics_dict, options=options)
    ...
    def fit_generator(self, datagen=None, x=None, y=None, batch_size=None, epochs=1, validation_data=None):
        ...
        if x is not None:
            ...
            history = self.keras_model.fit_generator(generator=datagen,
                                                     steps_per_epoch=int(datagen.image_filenames.shape[0] // batch_size),
                                                     epochs=epochs,
                                                     callbacks=callbacks,
                                                     verbose=1,
                                                     validation_data=validation_data,
                                                     validation_steps=int(validation_data.image_filenames.shape[0] // batch_size))
        return history
